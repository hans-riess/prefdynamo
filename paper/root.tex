...
% Let $\Fix(f) = \{ \pi \in \lattice: f(\pi) = \pi \}$ denote the set of fixed points.
% $\Pre(f) = \{ \pi \in \lattice : f(\pi) \preceq \pi\}$ the set of \emph{prefix points}, and $\Post(f) =\{ \pi \in \lattice : f(\pi) \succeq \pi\}$ the set of \emph{suffix points}. If they exist, let $\lfp(f)$ and $\gfp(f)$ denote the least and greatest fixed points, respectively.
...
% \begin{align}
%     \begin{aligned}
%         \bigmeet_{j \in J} \pi_j = \max\{\pi \in \lattice : \pi \preceq \pi_j~\forall j \in J\} \\
%         \bigjoin_{j \in J} \pi_j  = \min\{\pi \in \lattice : \pi \succeq \pi_j~\forall j \in J\}
