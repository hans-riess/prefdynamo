...
\begin{align}
    F_i(\boldsymbol{\pi}) = \varphi\Bigl(\pi_i, \Aggregate{\left[ \psi(\pi_j, \pi_i)\right]_{j \in \N_i^k}} \Bigr) \label{eq:message-passing}
\end{align}
...
\begin{proof}
    ...
    % Hence, by monotonicity of the height function, $\{h\bigl(\boldsymbol{\pi}(k) \bigr) \}_{k=0}^{\infty}$ is a monotonically non-increasing function bounded below by $h(\epsilon) = 0$.
\end{proof}
...
\SetKwComment{Comment}{/* }{ */}
\KwData{$N$ agents, $M$ alternatives; iterative map $F = (F_1,\dots,F_N)$ of preference dynamics.}
\KwResult{Equilibrium point $\pi^{\ast}$ of $F$}
