...
\author{Hans Riess$^{1}$~Gregory Henselman-Petrusek$^{2}$~Michael C.~Munger$^{3}$~Robert Ghrist$^{4}$ \\ Zachary I.~Bell$^{5}$~Michael M.~Zavlanos$^{1,6}$% <-this % stops a space
...
% We remain agnostic to the ``preference change'' debate (see \cite{stigler1977}).
...
% We do not assume $\A$ is finite, except to establish convergence guarantees (see Section \ref{sec:stable}).
...
% In all three examples, the class of relative orderings over the alternative set is restricted.
...
% For the purpose of illustration, we supply an example of a multi-agent system with preferences.

% \begin{example}
%     Suppose a team of five autonomous unmanned aerial vehicles (UAVs) are monitoring three regions.
%     % The agents set consists of the IDs of the UAVs, $\N = \{1,2,3,4,5\}$.
%     The alternative set consists of the regions, $\A = \{R_1,R_2,R_3\}$ that agents need to monitor. Preferences consist of relative rankings by individual UAVs of the regions, according to their level of safety, e.g.~f Agent 1 believes Region 1 is safer than Region 3, then $R_1 \prefers_1 R_3$.  The graph consists of an \emph{ad hoc} wireless communication network the UAVs form as they move from region to region.
% \end{example}
...
% For more details on lattice and order theory, see \cite{birkhoff1940}, or \cite{roman2008}, for a more recent exposition.
...
For lattices that are not finite, there is a distinction between the existence of binary meets and joins versus arbitrary meets and joins. Given $\{x_j\}_{j \in J} \subseteq \L$, a lattice is \emph{complete} if $\bigmeet_{j \in J} \pi_j = \max\{\pi \in \L : \pi \preceq \pi_j~\forall j \in J\}$ and $\bigjoin_{j \in J} \pi_j  = \min\{\pi \in \L : \pi \succeq \pi_j~\forall j \in J\}$ exist.
% \begin{align}
%     \begin{aligned}
...
% Thresholds for majorities are have been explored in economics \cite{buchanan1965calculus}.
...
% : if $a \prefers b$ is in the aggregate of preferences observed by an agent belongs to the agent's previous preferences, the comparison is kept, otherwise discarded.
...
% As a point of warning, the operations $\meet_\SS$ and $\join_\SS$ inside $\SS$ do not, in general, coincide with the operations $\meet$ and $\join$ inside $\prod_{i \in \N}  \Pref(\A)$.
...
% Note this notion of distance between preference relations does \emph{not} reflect agreement, only disagreement or lack thereof.
...
% The Dirichlet energy for two such graphs ($k=4,8$) are plotted in Fig.~\ref{fig:1}.
...
% We also observed the total level of disagreement (energy) was higher for graphs with higher connectivity. Both of these observations are explained by $F$ satisfying the inflationary property: agents can only include more comparisons in the next round and agents with more neighbors means more comparisons to choose from.
